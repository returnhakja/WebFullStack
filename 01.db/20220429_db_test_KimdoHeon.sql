...
SELECT * from test_detail_view;


-- 01.test_detail_view 에서 salary이 높은순으로 정렬하라
SELECT * from test_detail_view;

select salary from test_detail_view order by salary desc;


-- 02. manager_id 를 그룹화 하여 부서번호 숫자가 작은 순으로 정렬된 결과를 출력
-- step
SELECT * from test_detail_view;
-- Final
select manager_id from test_detail_view group by manager_id order by manager_id;

-- 03. test_detail_view 에서 급여가 3000 이상인 employee_id를 내림차순으로 출력
-- step
SELECT * from test_detail_view;
-- Final
select employee_id from test_detail_view group by salary HAVING salary >= 3000 order by employee_id desc;

-- 04. employees , departments 에서 department_id가 같은 employees_id , email, phone_number 출력
-- step
select * from employees;
select * from departments;
-- Final
...
-- 05. jobs의 AD_ASST의 employee_id, start_date, end_date 출력
-- step
select * from jobs;
select * from job_history;
...
from  job_history h
where h.job_id = (select jobs.job_id from jobs where jobs.job_id = 'AD_ASST');

-- 06. manager_id가 200인 id의 location_id와 city 출력
-- step
SELECT * from locations;
SELECT * from departments;
...
-- 07. departments , job_history 테이블을 이용해 department_id가 같으면 id, 시작데이터, 끝나는데이터 출력
-- step
select * from departments;
select * from job_history;
...
-- Final
DELIMITER $$ 
CREATE PROCEDURE Getdepar() 
BEGIN 
   select d.department_id , h.start_date , h.end_date
...
-- 08. jobs 테이블에 AB_CDE president 3000 5000 삽입하시오
...
-- Final
INSERT INTO jobs VALUES('AB_CDE', 'president', 3000,1, 5000,1);
select * from jobs;

-- 09. JOBS  테이블의 모든 id에게 max_salary를 30000으로 수정하시오.
SELECT * from jobs;
desc jobs;
UPDATE jobs SET max_salary = 30000.0;

select * from jobs;

-- 10. JOBS 테이블의 AB_CDE를 삭제 시키시오.
select * from jobs;
